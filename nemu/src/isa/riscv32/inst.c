...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, R(rd) = s->pc + 4;s->dnpc = s->pc+SEXT(imm,21)-4);
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, R(rd) = s->pc + 4;s->dnpc = ((src1 + SEXT(imm,12))&~1)-4);
  ...
  INSTPAT("??????? ????? ????? 000 ????? 11000 11", beq    , B, if(src1 == src2)  s->dnpc = s->pc + SEXT(imm,13)-4);
  INSTPAT("??????? ????? ????? 001 ????? 11000 11", bne    , B, if(src1 != src2)  s->dnpc = s->pc + SEXT(imm,13)-4);
  ...
  INSTPAT("??????? ????? ????? 101 ????? 11000 11", bge    , B, if((int32_t)src1>=(int32_t)src2)  s->dnpc = s->pc + SEXT(imm,13)-4);
  INSTPAT("0000000 ????? ????? 101 ????? 00100 11", srli   , I, R(rd) = src1>>imm);
  INSTPAT("??????? ????? ????? 111 ????? 11000 11", bgeu   , B, if((uint32_t)src1>=(uint32_t)src2)  s->dnpc = s->pc + SEXT(imm,13)-4);
